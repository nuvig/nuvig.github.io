  ...
  [
    ...
    [
      ...
      n('Aeromedical Factors', 'hf', 'How altitude and stress act on the body.',
        [
          (function(){ const hyp = n('Hypoxia', 'hf', 'Oxygen starvation — insidious, euphoric, deadly. Regulatory oxygen above 12,500 ft (after 30 min) and always above 14,000 ft.',
            [
    ...
    [
      (function(){ const eng = n('Engine & Power Loss', 'emerg', 'The prop stops or falters — fly the airplane first, then work the problem on the way down.',
        [
          n('Complete Power Loss', 'emerg', 'Pitch for best glide, pick a field, run the flow — fuel selector, mixture rich, boost pump, mags both, primer in, carb heat — then Mayday/7700, secure, and land. Aviate the whole way.'),
          n('Best Glide & Field Selection', 'emerg', 'Trim to L/Dmax for maximum range, judge wind and slope, commit to a field early and fly a normal-looking high-key/low-key pattern to it.', [], [['xBestGlideRef','L/Dmax']]),
          n('Partial Power & Rough Running', 'emerg', 'Diagnose fuel, air (carb ice), spark and mixture. Carb heat and a mixture sweep cure a surprising share of roughness — nurse it toward an airport, never trust it.'),
          n('Engine Failure After Takeoff', 'emerg', 'Low and slow with few options: lower the nose NOW to hold glide speed and land within a shallow arc ahead. The turn-back "impossible turn" stalls and spins the unwary — pick an altitude gate in advance.', [], [['xStallRef','avoid the stall/spin']]),
          n('Forced & Precautionary Landing', 'emerg', 'Forced = must land now; precautionary = choosing to land while you still can (weather, fuel, lost). Brief passengers, tighten belts, fuel/mags off short final, minimum energy at touchdown.'),
        ]); eng.id='xEngFailRef'; return eng; })(),
      n('Fire & Smoke', 'emerg', 'The one emergency where speed beats troubleshooting — cut the source, ventilate deliberately, and land as soon as possible.',
        [
          n('Engine Fire', 'emerg', 'Mixture idle-cutoff, fuel selector off, cabin heat/air off, then a forced landing — do not attempt a restart. A sideslip can keep flames off the cabin.'),
          n('Electrical Fire', 'emerg', 'Acrid smoke and a hot smell: master/battery off, kill the bus, land. If systems are essential, add items back one at a time to find the culprit.', [], [['xElecEmergRef','electrical system']]),
          n('Cabin Fire', 'emerg', 'Extinguisher (PASS), then ventilate — a fire fed by fresh air may mean closing vents first. Descend and land immediately; smoke is as dangerous as flame.'),
          (function(){ const co = n('Carbon Monoxide', 'emerg', 'Odorless exhaust from a cracked heater shroud — headache, sleepiness, then incapacitation. Heater off, fresh-air vents open, oxygen if available, and land.'); co.id='xCORef'; return co; })(),
        ]),
      n('System & Instrument Failures', 'emerg', 'Something quits — recognize the pattern, work around it, and land where help is.',
        [
          (function(){ const ef2 = n('Electrical Failure', 'emerg', 'Alternator warning / discharge: reduce load, run the checklist, and treat the battery as a countdown to essential-only. Know exactly what you lose when it goes dark.'); ef2.id='xElecEmergRef'; return ef2; })(),
          (function(){ const vp = n('Vacuum / Partial Panel', 'emerg', 'Gyros die slowly and lie convincingly — the classic killer. Cover them, fly the electric turn coordinator + ASI + altimeter, and get to VFR.', [], [['xGyroRef','lost gyros']]); vp.id='xVacEmergRef'; return vp; })(),
          n('Pitot-Static Failures', 'emerg', 'A blocked pitot or static port gives a predictable but deadly-wrong reading — cross-check attitude + power, select the alternate static source, and trust the picture that agrees.', [], [['xPitotInstRef','the instruments']]),
          n('Gear & Flap Malfunction', 'emerg', 'Gear that won\'t extend (manual/emergency extension, positive-g yaw), or asymmetric/split flaps — fly the airplane, run the checklist, and plan a longer, flatter arrival.'),
          n('Trim & Autopilot Runaway', 'emerg', 'A runaway trim or misbehaving autopilot: disconnect it (autopilot switch, then its breaker), hold the airplane with muscle, and re-trim by hand.'),
        ]),
      n('Loss of Control & Upset', 'emerg', 'The airplane departs controlled flight or points somewhere alarming — recover with the wings first.',
        [
          n('Stall / Spin Recovery', 'emerg', 'Stall: reduce AoA (nose down), full power, level the wings, minimum altitude loss. Spin: PARE — Power idle, Ailerons neutral, Rudder opposite the spin, Elevator forward.', [], [['xStallRef','the aerodynamics']]),
          n('Unusual Attitude Recovery', 'emerg', 'Nose-high near stall: add power, unload, level the wings. Nose-low overspeed: reduce power, level the wings, ease out. Bank before pitch, and believe the instruments.', [], [['xManeuverRef','respect the envelope']]),
          n('Spiral Dive', 'emerg', 'Airspeed and bank increasing, altitude unwinding: power idle, roll the wings level FIRST, then gently raise the nose — pulling before leveling only tightens it.'),
          n('Inadvertent VFR-into-IMC', 'emerg', 'The deadliest trap: commit to the instruments, a standard-rate 180° turn back toward VMC, resist the leans, and ask for help early.', [], [['xSpatialRef','disorientation'],['xADMRef','the decision chain']]),
        ]),
      n('Weather & Environmental', 'emerg', 'Getting out of conditions the airplane can\'t survive in.',
        [
          n('Inadvertent Icing Encounter', 'emerg', 'Without known-ice gear: pitot heat on, exit the conditions — climb to warmer air or descend below the freezing level, add power and speed, and carry extra on approach; never let it stall.', [], [['xIcingRef','airframe ice']]),
          n('Storm / Severe Turbulence', 'emerg', 'If trapped: slow to maneuvering speed, hold attitude (not altitude), lights up, avoid abrupt inputs, and turn to the shortest way out.', [], [['xTstormRef','the storm'],['xManeuverRef','fly Va']]),
          n('Wind Shear / Microburst Escape', 'emerg', 'On approach: full power, pitch to the attitude/flight-director for maximum climb, don\'t chase airspeed, and go around early. Never try to fly through a microburst.', [], [['xShearRef','the shear']]),
          n('Lost & Disoriented', 'emerg', 'The five Cs: Climb (for radio/radar/nav range), Communicate, Confess, Comply, Conserve. Use flight following, DF steers and every nav aid before fuel decides for you.', [], [['xCommRef','confess & ask']]),
        ]),
      n('Cabin & Passengers', 'emerg', 'The emergencies that happen behind the panel.',
        [
          n('Door or Window Open', 'emerg', 'Startling noise, rarely a real handling problem — fly the airplane, don\'t grab for it in a critical phase, and land to secure it. Most cabin doors can\'t be closed in flight.'),
          n('Passenger Incapacitation', 'emerg', 'Brief passengers before flight; if someone is ill or panicking, delegate tasks, use medical help relayed via ATC, and land at the nearest suitable airport.'),
          n('Hypoxia / Depressurization', 'emerg', 'Insidious oxygen starvation: don oxygen, descend to a safe altitude immediately, and treat any unexplained euphoria or fumbling as hypoxia until proven otherwise.', [], [['xHypoxiaRef','the physiology']]),
        ]),
      n('Emergency Comms & Equipment', 'emerg', 'Getting help, and the gear that summons it.',
        [
          n('Distress vs Urgency', 'emerg', 'MAYDAY×3 = distress (grave, immediate danger); PAN-PAN×3 = urgency (a problem, not yet dire). State who, what, where, intentions, and souls/fuel.', [], [['xCommRef','the comm layer']]),
          n('Squawk & 121.5', 'emerg', 'Transponder 7700 (general emergency), 7600 (lost comms), 7500 (hijack); 121.5 is the guard frequency monitored everywhere. ATC can help even before you\'ve reached them.'),
          n('ATC Assistance & DF Steer', 'emerg', 'ATC can give vectors, no-gyro steers, weather avoidance, light-gun signals and priority handling — declaring an emergency unlocks all of it. Ask, and accept the help.'),
          n('ELT & Survival', 'emerg', '406 MHz ELTs alert satellites with a GPS position; know how to trigger it manually, and carry what you\'d need to be found and to wait for rescue.'),
        ]),
      n('Priorities & Checklists', 'emerg', 'The discipline that makes all of the above survivable.',
        [
          n('Aviate, Navigate, Communicate', 'emerg', 'In that order, always. Fly the airplane first; a radio call or a checklist that steals attention from flying is how a survivable problem becomes a crash.'),
          n('Memory Items vs Read-Do', 'emerg', 'A handful of immediate actions are done from memory (engine fire, stall); everything else is read-and-do off the checklist once the airplane is stable and time allows.'),
          n('Decision-Making Under Pressure', 'emerg', 'Buy time (climb, hold, circle), name the problem, weigh the options, decide — then commit. Startle and tunnel vision are the real enemies.', [], [['xADMRef','ADM under load']]),
        ]),
    ]),
  ]);

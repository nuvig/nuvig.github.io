...
// dataset (Pi database, or every published day snapshot — up to the 62-day
// snapshot window). Day files are big, so the computed grid is cached in
// localStorage and reused for an hour (the snapshots update hourly anyway).
// ---------------------------------------------------------------------------
...
async function loadAllTimeGrid() {
  ...
  try {
    const now = Math.floor(Date.now() / 1000);
    // 60 days, not 62: KANPStatic.datesInRange caps at 62 iterated dates from
    // `start`, so asking for exactly 62 would push today past the cap
